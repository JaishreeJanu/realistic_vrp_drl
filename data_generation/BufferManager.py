...
class ReplayBuffer:
    ...
    _reserved_keys = (
        "obs", "act", "rew", "terminated", "truncated", "done", "obs_next", "info", "bl_rew",
        "policy"
    )
    _input_keys = (
        "obs", "act", "rew", "terminated", "truncated", "obs_next", "info", "policy", "bl_rew",
    )
    ...
    def __init__(
        ...
        **kwargs: Any,  # otherwise PrioritizedVectorReplayBuffer will cause TypeError
        ...
        self._ep_rew_bl: Union[float, np.ndarray]
        self.reset()
    ...
    def from_data(
        ...
        obs_next: h5py.Dataset, info: h5py.Dataset,
    ) -> "ReplayBuffer":
        size = len(obs)
        assert all(len(dset) == size for dset in [obs, act, rew, terminated,
                                                  truncated, done, obs_next, info]), \
    ...
    def reset(self, keep_statistics: bool = False) -> None:
        ...
        if not keep_statistics:
            self._ep_rew, self._ep_rew_bl, self._ep_len, self._ep_idx = 0.0, 0.0, 0, 0
    ...
    # Called by replay buffer manager
    def _add_index(self, rew: Union[float, np.ndarray], bl_rew: Union[float, np.ndarray],
        ...
        #print(f"\n ------------------ inside buffer")
        #print(f"old bl rew: {self._ep_rew_bl}")
        
        self._ep_rew += rew
        self._ep_rew_bl += bl_rew
        
        #print(f"got bl rew: {bl_rew}")
        #print(f"new bl rew: {self._ep_rew_bl}")
        
        self._ep_len += 1

        if done:
            result = ptr, self._ep_rew, self._ep_rew_bl, self._ep_len, self._ep_idx
            self._ep_rew, self._ep_rew_bl, self._ep_len, self._ep_idx = 0.0, 0.0, 0, self._index
            #print(f"index: {self._index}, buffer result: {result}")
            return result
        else:
            return ptr, self._ep_rew * 0.0, self._ep_rew_bl * 0.0, 0, self._ep_idx
    ...
    def add(
        ...
        # get ptr
        if stacked_batch:
            rew, done, bl_rew = batch.rew[0], batch.done[0], batch.info[0]["bl_rew"]
        else:
            rew, done, bl_rew = batch.rew, batch.done, batch.info["bl_rew"]
    
            
        ptr, ep_rew, ep_rew_bl, ep_len, ep_idx = list(
            map(lambda x: np.array([x]), self._add_index(rew, bl_rew, done))
        )
        ...
        try:
            ...
            batch.info["bl_rew"] = batch.info["bl_rew"].astype(float)
            ...
            self._meta[ptr] = batch
        return ptr, ep_rew, ep_rew_bl, ep_len, ep_idx
...
class ReplayBufferManager(ReplayBuffer):
    ...
    def add(
        ...
        ptrs, ep_lens, ep_rews, ep_rews_bl, ep_idxs = [], [], [], [], []
        
        for batch_idx, buffer_id in enumerate(buffer_ids):            
            ptr, ep_rew, ep_rew_bl, ep_len, ep_idx = self.buffers[buffer_id]._add_index(
                batch.rew[batch_idx], batch.info["bl_rew"][batch_idx], batch.done[batch_idx]
            )
            
            #if ep_rew_bl <= -10:
                #print(f"batch info in buffer: {batch.info[batch_idx]}")
                #print(f"buffer_id: {buffer_id}, raw bl_rew: {bl_rew}")
                
            #print(f"ep_rew_bl: {ep_rew_bl}")
            
            ptrs.append(ptr + self._offset[buffer_id])
            ep_lens.append(ep_len)
            ep_rews.append(ep_rew)
            ep_rews_bl.append(ep_rew_bl)
        ...
        #print("In add buffer:")
        #curr_ep_rews_bl = [buff._ep_rew_bl for buff in self.buffers if buff._ep_rew_bl <= -10]
        #print(f"current ep_rews_bl: {curr_ep_rews_bl}")
        
        #curr_ep_rews = [buff._ep_rew for buff in self.buffers if buff._ep_rew_bl <= -10]
        #print(f"\ncurrent ep_rews: {curr_ep_rews}")
        #print(f"rew: {ep_rew}, bl_rew: {ep_rew_bl}")
        
        return ptrs, np.array(ep_rews), np.array(ep_rews_bl), np.array(ep_lens), np.array(ep_idxs)
