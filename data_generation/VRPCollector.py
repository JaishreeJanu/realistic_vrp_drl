...
class Collector(object):
    ...
    def collect(
        ...
        while True:
            ...
            if random:
                ...
                if no_grad:
                    ...
                    result = self.policy(self.data.obs)
                #bl_rew=result.bl_rew
                #self.data.update(bl_rew = bl_rew)
            ...
            #print("\n\nAdding to buffer: (bl_act, bl_rew)")
            #print(self.data.obs)
            #print(self.data.info["bl_rew"])
            
            ptr, ep_rew, ep_rew_bl, ep_len, ep_idx = self.buffer.add(
            ...
            if np.any(done):
                ...
                episode_rews_bl.append(ep_rew_bl[env_ind_local])
            ...
            #print(self.data.info["bl_rew"])
            #print(f"curr ep rew_bl: {self.buffer.buffer_list[10]._ep_rew_bl}")
        
        # While loop for the episode generation ends here
        #print(f"Collected rewards from buffer:")
        #print(ep_rew)
        #print("baseline:")
        #print(ep_rew_bl)
        #print(episode_rews_bl)
        #print("---------")      
        ...
        if episode_count > 0:
            rews, bl_rews, lens, idxs = list(
                map(
                    np.concatenate,
                    [episode_rews, episode_rews_bl, episode_lens, episode_start_indices]
                )
            )
            rew_mean, rew_std = rews.mean(), rews.std()
            len_mean, len_std = lens.mean(), lens.std()
            bl_rew_mean, bl_rew_std = bl_rews.mean(), bl_rews.std()
            
            
        else:
            rews, bl_rews, lens, idxs = np.array([]), np.array([]), np.array([], int), np.array([], int)
            rew_mean = rew_std = len_mean = len_std = bl_rew_mean = bl_rew_std = 0
        
        #print(f"\n Collect step: {self.collect_step}, ep{self.collect_episode}, n_ep{n_episode}")
        #print(rews)
        #print(f"rew mean: {rew_mean}")
        #print(bl_rews)
        #print(f"bl_rew mean: {bl_rew_mean}")
