...
class VRPEnv(gym.Env):
    ...
    def __init__(self, instance,idx=0):
        ...
        self.length = len(self._instance)
        self._node_shape = self._instance[0]['node_features'].shape
        self._edge_shape = self._instance[0]['edge_features'].shape
        self.n_nodes = len(self._instance[0]['node_features'])  #Not counting depot node
        self.env_id = idx
        
        self.state = None
        self.action_space = spaces.Discrete(self.n_nodes)
        self.observation_space = spaces.Dict({
            'node_features': spaces.Box(low=0, high=1.0, shape=(self._node_shape)),
            'edge_features': spaces.Box(low=0, high=1.0, shape=(self._edge_shape)), 
    ...
    def reset(self):
        ...
        self._node_features= self._instance[idx]['node_features']
        self._edge_features = self._instance[idx]['edge_features']
