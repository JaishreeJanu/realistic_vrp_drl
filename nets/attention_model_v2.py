...
class GraphAttentionEncoder(nn.Module):
    def __init__(
        ...
        self.init_embed_edge = nn.Linear(self.graph_size, embed_dim) if self.graph_size is not None else None

        self.layers = nn.Sequential(*(
            MultiHeadAttentionLayer(n_heads, embed_dim, embed_dim, feed_forward_hidden, normalization)
            for _ in range(n_layers)
        ))

        ### Ilayda - Edge embeddings:
        self.g_embedding = nn.Linear(embed_dim, feed_forward_hidden)
        self.g_embedding1 = nn.Linear(feed_forward_hidden, feed_forward_hidden)
        self.g_embedding2 = nn.Linear(feed_forward_hidden, embed_dim)
        ###

        
        
    def forward(self, nodes_x, edges_x, mask=None): ### Ilayda - removed "x" and added "nodes_x" and "edges_x" instead

        assert mask is None, "TODO mask not yet supported!"

        # Batch multiply to get initial embeddings of nodes and edges
        nodes_h = self.init_embed(nodes_x.view(-1, nodes_x.size(-1))).view(*nodes_x.size()[:2], -1) if self.init_embed is not None else nodes_x 
        
        # edges_h = self.init_embed_edge(edges_x.view(-1, edges_x.size(-1))).view(*edges_x.size()[:2], -1) if self.init_embed_edge is not None else edges_x 
        # print(nodes_h.shape)
        
        # Horizontal stacking of nodes and edge features to create MHA input features and then applying attention mechanism
        h = nodes_h + edges_x
...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.init_embed_edge = nn.Linear(self.graph_size+1, embedding_dim)
    ...
    def encode_edges(self, edges_x, embedding_dim):
        # We will encode edges into a vector after some conv operations, instead of flatten and linear transformation
        Conv1 = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(1, self.graph_size), stride=1),
        h = Conv1(edges_x)
        
        Conv2 = nn.ReLU(nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(1, -1), stride=1), inplace=True)
        h = Conv2(h)
        h = torch.flatten(h)
        
        out_layer = nn.Linear(self.graph_size, embedding_dim)       
        return edges_h
    ...
    def forward(self, batch_obs):
        ...
        edges_x = torch.tensor(batch_obs["edge_features"], dtype=torch.float32).to(device)
        #print(nodes_x.shape)
        #print(edges_x.shape)
        #
        #print(self.init_embed)
        #print(self.init_embed_edge)
        ...
        if self.checkpoint_encoder and self.training:  # Only checkpoint if we need gradients
            ...
            graph_embeddings, _ = self.embedder(node_embeddings, edge_embeddings)  ### Ilayda - added "edges" in the embedder
        
        logits = self._inner(batch_obs, graph_embeddings)
        return logits
