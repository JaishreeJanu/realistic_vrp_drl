...
class BaseTrainer(ABC):
    ...
    def __init__(
        ...
        self.is_run = False
        self.last_rew, self.last_rew_bl, self.last_len = 0.0, 0.0, 0
        ...
        with progress(
            ...
            while t.n < t.total and not self.stop_fn_flag:
                ...
                if self.train_collector is not None:
                    ...
                    #print(f"\ntrain step: epoch{self.epoch}--env_step{self.env_step}")
                    #print(data)
                    #print(result)
    ...
    def train_step(self) -> Tuple[Dict[str, Any], Dict[str, Any], bool]:
        ...
        if result["n/ep"] > 0 and self.reward_metric:
            ...
            bl_rew = self.reward_metric(result["bl_rews"])
            result.update(rews=rew, rew=rew.mean(), rew_std=rew.std(), bl_rews=bl_rew, bl_rew=bl_rew.mean(), bl_rew_std=bl_rew.std())
        ...
        self.last_rew_bl = result["bl_rew"] if result["n/ep"] > 0 else self.last_rew_bl
        self.last_len = result["len"] if result["n/ep"] > 0 else self.last_len
        
        #print("\n Train step:")
        #print("last rew: ", result["rew"], self.last_rew)
        #print("last bl_rew: ",result["bl_rew"], self.last_rew_bl)
        #print(result)
        
        data = {
            "env_step": str(self.env_step),
            "rew": f"{self.last_rew:.2f}",
            "bl_rew": f"{self.last_rew_bl:.2f}",
